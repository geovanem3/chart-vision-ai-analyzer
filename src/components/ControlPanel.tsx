    ...
    try {
      ...
      setAnalysisResults({
        ...
        detailedMarketContext: {
          phase: marketContext?.phase || 'lateral',
          sentiment: marketContext?.sentiment || 'neutro',
          strength: marketContext?.strength || 'moderada',
          description: marketContext?.description || 'Análise de contexto não disponível',
          marketStructure: marketContext?.marketStructure || 'indefinido',
          breakoutPotential: marketContext?.breakoutPotential || 'médio',
          momentumSignature: marketContext?.momentumSignature || 'estável',
          institutionalBias: 'neutro',
          volatilityState: volatilityData?.isHigh ? 'alta' : 'normal',
          liquidityCondition: 'normal',
          timeOfDay: new Date().getHours() < 12 ? 'manhã' : 'tarde',
          trend: patterns.length > 0 && patterns[0].action === 'compra' ? 'alta' : 'baixa'
        },
